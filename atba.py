import math
from Reinforcement.State import GameInfo
import numpy as np
...
class Agent:
    def __init__(self, name, team, index):
        self.MATRIX_SAVE_FILE = 'saved array.npy'
        self.name = name
        self.team = team  # 0 towards positive goal, 1 towards negative goal.
        self.index = index
        self.jump = False
        f = Path('saved array.npy')
        if f.exists():
            self.decision_matrix = np.load(self.MATRIX_SAVE_FILE)
        else:
            self.decision_matrix = np.random.rand(44, 8)
            np.save(self.MATRIX_SAVE_FILE, self.decision_matrix)
    ...
    def get_output_vector(self, game_tick_packet):
        ...
        output_matrix = np.dot(state.get_input_matrix(), self.decision_matrix)
